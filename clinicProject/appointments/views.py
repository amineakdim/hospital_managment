...
def doBook(request):
    ...
    if hasGroup(user, 'secretaire'):
        ...
        appointment = Appointment(patient=patient, doctor=doctor, secretaire=request.user, case=c, appointment_time=appointment_time)
        appointment.save()
        messages.add_message(request, messages.INFO, 'Rendez-vous enregistré! ')
    else:
        messages.add_message(request, messages.WARNING, 'Accès interdit.')
...
def doChange(request):
    user = request.user
    if hasGroup(user, 'secretaire'):
        appointment = Appointment.objects.get(pk=(request.POST.get('id')))
        appointment.doctor = User.objects.get(username=request.POST.get('doctor', ''))
        appointment_time = request.POST.get('appointment_date')+'T'+request.POST.get('appointment_time')
        appointment_time = datetime(*[int(v) for v in appointment_time.replace('T', '-').replace(':', '-').split('-')])
        appointment.appointment_time = appointment_time
        appointment.secretaire = request.user
        appointment.save()
    messages.add_message(request, messages.INFO, 'Rendez-vous mis à jour avec succès.')
    return HttpResponseRedirect('/appointments/')
